...
func (g *Gun) shootStep(step *Call, sample *netsample.Sample, ammoName string, templateVars map[string]any, requestVars map[string]any, stepLogID string) error {
	...
	for _, postProcessor := range step.Postprocessors {
		...
		if g.gun.DebugLog {
			g.gun.GunDeps.Log.Debug("Postprocessor variables", zap.Any(fmt.Sprintf(".resuest.%s.postprocessor", step.Name), pp))
		}
	}
	if out != nil {
		// Postprocessor
...
func (g *Gun) buildLogID(idBuilder *strings.Builder, tag string, ammoID uint64, rnd string) {
	idBuilder.Reset()
